...
float rotBuzzHead = 0.0f, rotBuzzLeftArm = 0.0f, rotBuzzLeftForeArm = 0.0f, rotBuzzLeftHand = 0.0f, rotBuzzRightArm = 0.0f, rotBuzzRightForeArm = 0.0f, rotBuzzRightHand = 0.0f;
int modelSelected = 0;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	//modelBuzzLeftWing1.loadModel("../models/buzz/buzzlightyWing1.obj");
	//modelBuzzLeftWing2.loadModel("../models/buzz/buzzlightyWing2.obj");
...
bool processInput(bool continueApplication) {
	...
	if (modelSelected == 4 && glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS)
		modelMatrixBuzz = glm::translate(modelMatrixBuzz, glm::vec3(-0.02, 0.0, 0.002));
	else if (modelSelected == 4 && glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
		modelMatrixBuzz = glm::translate(modelMatrixBuzz, glm::vec3(0.02, 0.0, -0.02));
	*/
	// agregar las rotaciones de todas las extremidades
...
void applicationLoop() {
	...
	//modelMatrixBuzz = glm::translate(modelMatrixDart, glm::vec3(15.0, 0.0, 20.0));
	...
	while (psi) {
		...
		*******************************************/
		...
		modelMatrixLamboFrontLeftWheel = glm::translate(modelMatrixLamboFrontLeftWheel, glm::vec3(0.773214, 0.377732, 1.39886));
		modelMatrixLamboFrontLeftWheel = glm::rotate(modelMatrixLamboFrontLeftWheel, -rotWheelsYLambo, glm::vec3(0, 1, 0));
		modelMatrixLamboFrontLeftWheel = glm::rotate(modelMatrixLamboFrontLeftWheel, rotWheelsXLambo, glm::vec3(1.0, 0, 0));
		modelMatrixLamboFrontLeftWheel = glm::translate(modelMatrixLamboFrontLeftWheel, glm::vec3(-0.773214, -0.377732, -1.39886));
		...
		modelMatrixLamboFrontRightWheel = glm::translate(modelMatrixLamboFrontRightWheel, glm::vec3(-0.758588, 0.377732, 1.39886));
		modelMatrixLamboFrontRightWheel = glm::rotate(modelMatrixLamboFrontRightWheel, -rotWheelsYLambo, glm::vec3(0, 1, 0));
		modelMatrixLamboFrontRightWheel = glm::rotate(modelMatrixLamboFrontRightWheel, rotWheelsXLambo, glm::vec3(1.0, 0, 0));
		modelMatrixLamboFrontRightWheel = glm::translate(modelMatrixLamboFrontRightWheel, glm::vec3(0.758588, -0.377732, -1.39886));
		...
		modelMatrixLamboRearLeftWheel = glm::translate(modelMatrixLamboRearLeftWheel, glm::vec3(0.764462, 0.397564, -1.60137));
		modelMatrixLamboRearLeftWheel = glm::rotate(modelMatrixLamboRearLeftWheel, rotWheelsXLambo, glm::vec3(1.0, 0, 0));
		modelMatrixLamboRearLeftWheel = glm::translate(modelMatrixLamboRearLeftWheel, glm::vec3(-0.764462, -0.397564, 1.60137));
		...
		modelMatrixBuzzBody = glm::scale(modelMatrixBuzz,glm::vec3(3.0f));
		...
		*/
		...
		if(modelSelected == 1 && record){
			matrixDartJoints.push_back(rotDartHead); // 0
			
			matrixDartJoints.push_back(rotDartLeftArm); // 1
			matrixDartJoints.push_back(rotDartLeftHand); // 2
			matrixDartJoints.push_back(rotDartLeftLeg); // 3
			
			matrixDartJoints.push_back(rotDartRightArm); // 4
			matrixDartJoints.push_back(rotDartRightHand); // 5
			matrixDartJoints.push_back(rotDartRightLeg); // 6
		...
		switch (stateLambo) {
			...
			case 2: // giros
				if (contLambo > 0) {
					...
					if (rotWheelsYLambo > 0.3f)
						rotWheelsYLambo = 0.3f;
				...
				break;
			case 3: // se detiene el lambo y abre puerta
				if (dorRotCount > 75.0f && contLambo > 0)
					dorRotCount = 75.0f;
				dorRotCount+=0.5f;
				break;
